...
La recuperación de un sync BLOCKED tiene dos caminos cerrados. Online, continuar desde estado central usa `GET /sheet` y `TakeOverMatchSheet`; la cola anterior se conserva íntegra y queda fuera del replay de la nueva sesión. Offline sólo puede reconstruirse una vista de solo consulta hasta antes del evento rechazado cuando `/sync` lo identifica mediante `eventUuid` y `localSequence` y MatchEngine reproduce determinísticamente el rechazo. El evento y sus descendientes se conservan sin aplicar y el runtime permanece BLOCKED. No se crean sesiones sólo en cliente, no se renumeran ni reutilizan secuencias y continuar offline mediante branching/rebase permanece pendiente de protocolo.
