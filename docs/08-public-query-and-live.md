...
Fixture / Resultados es una única vista del recurso público de fixture. Sus filtros son locales sobre la proyección ya cargada: `Todos` conserva todos los estados publicados, `Próximos` muestra sólo `SCHEDULED` y `Resultados` sólo `FINISHED`; los demás estados no se reinterpretan ni se incluyen en esos dos filtros. Cuando hay más de una fase se puede acotar por fase y, dentro de una fase con grupos, por grupo; cambiar la fase descarta el grupo anterior. Los filtros preservan fase → ronda, fase → grupo → ronda y playoff → serie → partidos, sin contenedores vacíos ni rondas artificiales para series. El estado se puede compartir mediante `matches=upcoming|results`, `phase` y `group` en la query de `/competitions/{id}/fixture`; la ausencia de esos parámetros equivale a Todos y todas las fases/grupos.

Posiciones consume exclusivamente la tabla canónica de `GET /api/public/competitions/{id}/standings`. Muestra un único scope por vez: sin parámetros selecciona determinísticamente la primera tabla recibida; si hay varias fases publicables permite `phase`, y si la fase seleccionada tiene tablas de grupos permite `group`. Los identificadores de query son los IDs públicos `phaseId` y `phaseGroupId`; una selección inexistente se presenta como la primera tabla válida. Cada grupo conserva su tabla independiente y React sólo representa `Position`, `IsTied`, el orden y las métricas recibidas, sin recalcular puntuación, ratios ni desempates. En móvil permanecen visibles posición, equipo, PJ, PG, PP y puntos de tabla; sets y ratios son métricas secundarias ocultables. Los ratios nulos se presentan como «—».

Playoffs usa la ruta contextual `/competitions/{id}/playoffs` y representa las fases y series en el orden público recibido. Cada serie muestra su código/nombre, estado, ambos participantes, `seriesWins`, `winsRequired`, ventaja inicial si existe y sólo sus Matches materializados. Un participante no resuelto conserva la fuente pública `SERIES_WINNER` o `SERIES_LOSER` con su código de serie; si no hay equipo ni fuente se muestra «Por definir». El ganador se destaca únicamente cuando `winnerTeamEntryId` lo identifica. En desktop, cuando las fuentes relacionan explícitamente series de una misma fase, las series origen ocupan la columna izquierda y sus destinos la derecha; los conectores de ganador y perdedor se derivan sólo de esas fuentes y tienen jerarquía visual distinta. Sin dependencias públicas válidas se conserva una grilla de series sin conexiones. Desde 320 px todo pasa a una columna vertical, sin conectores complejos, scroll horizontal ni partidos ficticios.
