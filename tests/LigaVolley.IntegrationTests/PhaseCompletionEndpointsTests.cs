...
public sealed class PhaseCompletionEndpointsTests:IClassFixture<LigaVolleyApiFactory>
{
    ...
    public async Task PreviewIsReadOnlyAndCompleteIsTransactionalIncrementalAndIdempotent()
    {
        ...
        var format=await Create<CompetitionFormatDto>("/api/admin/competition-formats",new CreateCompetitionFormatRequest($"PC_{suffix}",$"Completion {suffix}",null,4,4,definition));await factory.Client.PatchAsJsonAsync($"/api/admin/competition-formats/{format.CompetitionFormatId}/active",new{active=true},Json);
