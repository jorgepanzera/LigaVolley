...
public sealed class PublicQueryEndpointsTests(LigaVolleyApiFactory factory):IClassFixture<LigaVolleyApiFactory>
{
    ...
    public async Task Draft_is_transitively_hidden_and_scheduled_competition_is_navigable()
    {
        ...
        var definition=new CompetitionFormatDefinitionDto([new("REG","Regular",PhaseType.RoundRobin,PhaseRole.Regular,1,1,FixtureMode.BalancedRandom,[],[]),new("GROUPS","Groups",PhaseType.GroupStage,PhaseRole.Championship,2,null,null,[new("CHAMP","Championship",GroupRole.Championship,1,1,FixtureMode.BalancedRandom,CarryOverMode.None),new("RELEG","Relegation",GroupRole.Relegation,2,1,FixtureMode.BalancedRandom,CarryOverMode.None)],[])],[],[new(3,0,3,0),new(3,1,3,0),new(3,2,2,1)],[new(1,TiebreakCriterion.TablePoints,SortDirection.Desc)],[]);var format=await Create<CompetitionFormatDto>("/api/admin/competition-formats",new CreateCompetitionFormatRequest($"PUB_{suffix}","Public",null,2,2,definition));await factory.Client.PatchAsJsonAsync($"/api/admin/competition-formats/{format.CompetitionFormatId}/active",new{active=true},Json);var competition=await Create<CompetitionDto>("/api/admin/competitions",new CreateCompetitionRequest($"Public {suffix}",season.SeasonId,division.DivisionId,CompetitionPeriodType.Annual,null,null,new(CompetitionStructureSourceType.Format,format.CompetitionFormatId,null)));
        Assert.Equal(HttpStatusCode.NotFound,(await factory.Client.GetAsync($"/api/public/competitions/{competition.CompetitionId}")).StatusCode);Assert.Equal(HttpStatusCode.NotFound,(await factory.Client.GetAsync($"/api/public/seasons/{season.SeasonId}/home")).StatusCode);Assert.DoesNotContain((await factory.Client.GetFromJsonAsync<PublicSeasonDto[]>("/api/public/seasons",Json))!,x=>x.SeasonId==season.SeasonId);
        ...
        var seasons=(await factory.Client.GetFromJsonAsync<PublicSeasonDto[]>("/api/public/seasons",Json))!;Assert.Contains(seasons,x=>x.SeasonId==season.SeasonId);Assert.Equal(seasons.OrderByDescending(x=>x.Year).Select(x=>x.SeasonId),seasons.Select(x=>x.SeasonId));var detail=await factory.Client.GetFromJsonAsync<PublicCompetitionDto>($"/api/public/competitions/{competition.CompetitionId}",Json);Assert.Equal(2,detail!.Teams.Count);Assert.Equal(2,detail.Phases.Count);var fixture=await factory.Client.GetFromJsonAsync<PublicCompetitionFixtureDto>($"/api/public/competitions/{competition.CompetitionId}/fixture",Json);var regular=Assert.Single(fixture!.Phases.Where(x=>x.Code=="REG"));Assert.Single(regular.Rounds);Assert.Single(fixture.Phases.Where(x=>x.Code=="GROUPS"));var standings=await factory.Client.GetFromJsonAsync<PublicCompetitionStandingsDto>($"/api/public/competitions/{competition.CompetitionId}/standings",Json);Assert.Equal(3,standings!.Tables.Count);Assert.Equal(2,standings.Tables[0].Rows.Count);Assert.All(standings.Tables.Skip(1),table=>Assert.Empty(table.Rows));
        var home=await factory.Client.GetFromJsonAsync<PublicSeasonHomeDto>($"/api/public/seasons/{season.SeasonId}/home",Json);Assert.Equal(season.SeasonId,home!.Season.SeasonId);Assert.Single(home.ActiveCompetitions);Assert.Empty(home.LiveMatches);Assert.Empty(home.UpcomingMatches);Assert.Empty(home.RecentResults);Assert.Empty(home.FinishedCompetitions);
        await using(var scope=factory.Services.CreateAsyncScope()){var db=scope.ServiceProvider.GetRequiredService<LigaVolleyDbContext>();var match=await db.Matches.SingleAsync(x=>x.CompetitionId==competition.CompetitionId);match.Schedule(DateTime.UtcNow,null);match.Start();await db.SaveChangesAsync();}
        home=await factory.Client.GetFromJsonAsync<PublicSeasonHomeDto>($"/api/public/seasons/{season.SeasonId}/home",Json);Assert.Single(home!.LiveMatches);Assert.Equal(competition.CompetitionId,home.LiveMatches[0].CompetitionId);Assert.Equal(0,home.LiveMatches[0].Score!.HomeSets);
        using var homeJson=JsonDocument.Parse(await factory.Client.GetStringAsync($"/api/public/seasons/{season.SeasonId}/home"));var matchJson=homeJson.RootElement.GetProperty("liveMatches")[0];Assert.False(matchJson.TryGetProperty("roster",out _));Assert.False(matchJson.TryGetProperty("officials",out _));Assert.False(matchJson.TryGetProperty("court",out _));
    ...
    {
        using var doc=JsonDocument.Parse(await factory.Client.GetStringAsync("/swagger/v1/swagger.json"));var paths=doc.RootElement.GetProperty("paths");foreach(var path in new[]{"/api/public/seasons","/api/public/seasons/{seasonId}/home","/api/public/competitions","/api/public/competitions/{competitionId}","/api/public/competitions/{competitionId}/fixture","/api/public/competitions/{competitionId}/standings","/api/public/matches/{matchId}","/api/public/matches/{matchId}/live"})Assert.True(paths.TryGetProperty(path,out _),path);
        var seasonHome=paths.GetProperty("/api/public/seasons/{seasonId}/home").GetProperty("get");Assert.Contains("bounded public summaries",seasonHome.GetProperty("description").GetString());
        ...
        var homeSchema=schemas.GetProperty(nameof(PublicSeasonHomeDto)).GetProperty("properties");Assert.Equal(new[]{"activeCompetitions","finishedCompetitions","liveMatches","recentResults","season","upcomingMatches"},homeSchema.EnumerateObject().Select(x=>x.Name).Order());
    }
