...
public sealed class PublicQueryEndpointsTests(LigaVolleyApiFactory factory):IClassFixture<LigaVolleyApiFactory>
{
    ...
    public async Task Draft_is_transitively_hidden_and_scheduled_competition_is_navigable()
    {
        ...
        var definition=new CompetitionFormatDefinitionDto([new("REG","Regular",PhaseType.RoundRobin,PhaseRole.Regular,1,1,FixtureMode.BalancedRandom,[],[])],[],[new(3,0,3,0)],[new(1,TiebreakCriterion.TablePoints,SortDirection.Desc)],[]);var format=await Create<CompetitionFormatDto>("/api/admin/competition-formats",new CreateCompetitionFormatRequest($"PUB_{suffix}","Public",null,2,2,definition));var competition=await Create<CompetitionDto>("/api/admin/competitions",new CreateCompetitionRequest($"Public {suffix}",season.SeasonId,division.DivisionId,CompetitionPeriodType.Annual,null,null,new(CompetitionStructureSourceType.Format,format.CompetitionFormatId,null)));
        ...
        for(var i=1;i<=2;i++){var team=await Create<TeamDto>("/api/admin/teams",new CreateTeamRequest($"Public {suffix} {i}",Gender.Female,null));await Create<TeamEntryDto>($"/api/admin/competitions/{competition.CompetitionId}/entries",new AddTeamEntryRequest(team.TeamId,(short)i));}var generated=await factory.Client.PostAsJsonAsync($"/api/admin/competitions/{competition.CompetitionId}/fixture/generate",new GenerateFixtureRequest(42),Json);generated.EnsureSuccessStatusCode();
