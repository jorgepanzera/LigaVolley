...
public sealed partial class MatchEngineEndpointsTests(LigaVolleyApiFactory factory):IClassFixture<LigaVolleyApiFactory>
{
    ...
    public async Task Offline_sync_overlap_concurrency_takeover_and_abandoned_session_are_consistent()
    {
        ...
        var invalidEventUuid=Guid.NewGuid();var sportingRejection=await factory.Client.PostAsJsonAsync($"/api/scorer/matches/{x.MatchId}/sync",Sync(started.Snapshot,[(invalidEventUuid,2L,ScorerSyncEventType.Substitution,new{setNumber=1,side=MatchSide.Home,playerOutMatchPlayerId=x.Home[0],playerInMatchPlayerId=x.Home[7]})]),Json);Assert.Equal(HttpStatusCode.Conflict,sportingRejection.StatusCode);using var sportingProblem=JsonDocument.Parse(await sportingRejection.Content.ReadAsStringAsync());Assert.Equal("substitution_player_is_libero",sportingProblem.RootElement.GetProperty("code").GetString());Assert.Equal(invalidEventUuid,sportingProblem.RootElement.GetProperty("eventUuid").GetGuid());Assert.Equal(2, sportingProblem.RootElement.GetProperty("localSequence").GetInt64());
