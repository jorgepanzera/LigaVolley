...
public sealed partial class MatchEngineEndpointsTests(LigaVolleyApiFactory factory):IClassFixture<LigaVolleyApiFactory>
{
    ...
    public async Task Full_engine_finishes_three_zero_corrects_tracks_and_closes_idempotently()
    {
        ...
        var libero=await Post<MatchEngineCommandResult>($"/api/scorer/matches/{x.MatchId}/sets/1/libero/enter",new LiberoEnterRequest(Guid.NewGuid(),x.Home[7],x.Home[0],["libero_service_not_allowed"]));Assert.Contains(libero.State.HomeCourtState,p=>p.EffectiveMatchPlayerId==x.Home[7]&&p.IsLiberoReplacement);
    ...
    public async Task Offline_sync_overlap_concurrency_takeover_and_abandoned_session_are_consistent()
    {
        ...
        var invalidEventUuid=Guid.NewGuid();var sportingRejection=await factory.Client.PostAsJsonAsync($"/api/scorer/matches/{x.MatchId}/sync",Sync(started.Snapshot,[(invalidEventUuid,2L,ScorerSyncEventType.Substitution,new{setNumber=1,side=MatchSide.Home,playerOutMatchPlayerId=x.Home[0],playerInMatchPlayerId=x.Away[7]})]),Json);Assert.Equal(HttpStatusCode.Conflict,sportingRejection.StatusCode);using var sportingProblem=JsonDocument.Parse(await sportingRejection.Content.ReadAsStringAsync());Assert.Equal("invalid_substitution",sportingProblem.RootElement.GetProperty("code").GetString());Assert.Equal(invalidEventUuid,sportingProblem.RootElement.GetProperty("eventUuid").GetGuid());Assert.Equal(2, sportingProblem.RootElement.GetProperty("localSequence").GetInt64());
