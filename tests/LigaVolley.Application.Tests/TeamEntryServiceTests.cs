...
public sealed class TeamEntryServiceTests
{
    ...
    [Fact] public async Task Add_RejectsTeamWithDifferentGender() { var f=Fixture(); var other=new Team("Other",Gender.Male,null); var teams=new FakeTeamRepository(); teams.Seed(7,other); var service=new TeamEntryService(f.Entries,new FakeCompetitionRepository(),teams,f.Unit); var competitions=new FakeCompetitionRepository(); competitions.Seed(1,f.Competition); service=new TeamEntryService(f.Entries,competitions,teams,f.Unit); var ex=await Assert.ThrowsAsync<ResourceConflictException>(()=>service.AddAsync(1,new(7,null),default)); Assert.Equal("team_gender_mismatch",ex.Code); }
