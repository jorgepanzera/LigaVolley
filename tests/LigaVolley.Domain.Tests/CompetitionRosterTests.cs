...
public sealed class CompetitionRosterTests
{
 ...
 [Fact]public void Inactive_players_do_not_count_toward_limit(){var r=Roster();for(var i=0;i<15;i++)r.AddPlayer(Player(i),null,PlayerRole.Setter);r.ChangePlayerStatus(r.Players[0],CompetitionRosterMemberStatus.Inactive);r.AddPlayer(Player(20),null,PlayerRole.Setter);Assert.Equal(15,r.Players.Count(x=>x.Status==CompetitionRosterMemberStatus.Active));}
 [Fact]public void Rejects_sixteenth_active_player(){var r=Roster();for(var i=0;i<15;i++)r.AddPlayer(Player(i),null,PlayerRole.Setter);Assert.Throws<DomainValidationException>(()=>r.AddPlayer(Player(20),null,PlayerRole.Setter));}
 [Fact]public void Rejects_third_libero_and_duplicate_active_jersey(){var r=Roster();r.AddPlayer(Player(1),1,PlayerRole.Libero);r.AddPlayer(Player(2),2,PlayerRole.Libero);Assert.Throws<DomainValidationException>(()=>r.AddPlayer(Player(3),3,PlayerRole.Libero));Assert.Throws<DomainValidationException>(()=>r.AddPlayer(Player(4),1,PlayerRole.Setter));}
 [Fact]public void Rejects_third_active_coach_and_closed_edits(){var r=Roster();r.AddStaff(Coach());r.AddStaff(Coach());Assert.Throws<DomainValidationException>(()=>r.AddStaff(Coach()));r.Activate();r.Close();Assert.Throws<DomainValidationException>(()=>r.AddPlayer(Player(9),null,PlayerRole.Setter));}
