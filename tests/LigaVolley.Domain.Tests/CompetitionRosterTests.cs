...
public sealed class CompetitionRosterTests
{
 ...
 [Fact]public void Allows_any_number_of_habitual_liberos_within_the_general_player_limit(){var r=Roster();for(var i=0;i<15;i++)r.AddPlayer(Player(i),PlayerRole.Libero);Assert.Equal(15,r.Players.Count(x=>x.Status==CompetitionRosterMemberStatus.Active));Assert.Throws<DomainValidationException>(()=>r.AddPlayer(Player(20),PlayerRole.Libero));}
 [Fact]public void Allows_a_null_habitual_function(){var r=Roster();r.AddPlayer(Player(1),null);Assert.Null(r.Players.Single().Role);}
