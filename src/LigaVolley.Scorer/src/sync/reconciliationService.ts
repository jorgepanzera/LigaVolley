...
export async function reconcile(
  ...
  matchId: number,
  server: ServerSheetSnapshot,
  lastAccepted: number,
) {
  await database.transaction(
    'rw',
    database.matchSheets,
    database.sessions,
    database.snapshots,
    database.events,
    async () => {
      const sheet = await database.matchSheets.get(matchId);
      if (!sheet) throw new Error('offline_no_local_match');
      const session =
        (await database.sessions.get(server.session.sessionUuid)) ??
        (await database.sessions.where('matchId').equals(matchId).toArray()).find(
          (x) => x.status === 'ACTIVE',
        );
      ...
      const remaining = pending
        .filter((x) => x.sequence > lastAccepted && x.syncStatus !== 'ACCEPTED')
        .sort((a, b) => a.sequence - b.sequence);
      const state = replay(base, remaining);
      await database.matchSheets.update(matchId, {
        bootstrap: server,
      ...
      await database.snapshots.put({
        matchId,
        sheetUuid: sheet.sheetUuid,
        sessionUuid: session.sessionUuid,
        basedOnAcceptedSequence: lastAccepted,
        state,
        updatedAt: new Date().toISOString(),
      });
    },
