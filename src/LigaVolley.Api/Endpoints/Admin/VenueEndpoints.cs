...
internal static class VenueEndpoints
{
    public static IEndpointRouteBuilder MapVenueEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        // Example: GET /api/admin/venues?active=true
        group.MapGet("/", async (bool? active, VenueService service, CancellationToken ct) => Results.Ok(await service.ListAsync(active, ct)));
