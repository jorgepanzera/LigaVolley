...
internal static class CompetitionFormatEndpoints
{
    public static IEndpointRouteBuilder MapCompetitionFormatEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/", async (bool? active, short? teamCount, CompetitionFormatService service, CancellationToken ct) => Results.Ok(await service.ListAsync(active, teamCount, ct)));

        // Example: GET /api/admin/competition-formats/1
        group.MapGet("/{id:int}", async (int id, CompetitionFormatService service, CancellationToken ct) => Results.Ok(await service.GetAsync(id, ct)));
        ...
        group.MapPost("/", async (CreateCompetitionFormatRequest request, CompetitionFormatService service, CancellationToken ct) => { var result = await service.CreateAsync(request, ct); return Results.Created($"/api/admin/competition-formats/{result.CompetitionFormatId}", result); });
        ...
        group.MapPut("/{id:int}", async (int id, UpdateCompetitionFormatRequest request, CompetitionFormatService service, CancellationToken ct) => Results.Ok(await service.UpdateAsync(id, request, ct)));

        // Example: POST /api/admin/competition-formats/1/clone
        // Body: { "code": "RR8_V2", "name": "Round Robin 8 v2", "description": "Independent variant" }
        group.MapPost("/{id:int}/clone", async (int id, CloneCompetitionFormatRequest request, CompetitionFormatService service, CancellationToken ct) => { var result = await service.CloneAsync(id, request, ct); return Results.Created($"/api/admin/competition-formats/{result.CompetitionFormatId}", result); });

        // Example: PATCH /api/admin/competition-formats/1/active
        // Body: { "active": false }
        group.MapPatch("/{id:int}/active", async (int id, SetActiveRequest request, CompetitionFormatService service, CancellationToken ct) => Results.Ok(await service.SetActiveAsync(id, request.Active, ct)));
        ...
        group.MapPost("/validate", async (ValidateCompetitionFormatRequest request, CompetitionFormatService service) => Results.Ok(await service.ValidateAsync(request)));
        return endpoints;
    }
}
