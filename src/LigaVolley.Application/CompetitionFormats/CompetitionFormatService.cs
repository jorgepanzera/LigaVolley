...
public sealed class CompetitionFormatService(ICompetitionFormatRepository repository, IUnitOfWork unitOfWork)
{
    ...
    {
        var result = CompetitionFormatDefinitionFactory.Validate(request.MinTeams,request.MaxTeams,request.Definition,request.Code,request.Name);
        var errors = result.Errors.ToList();
        foreach (var (path, value) in new[] { ("maxSubstitutionsPerSet", request.MaxSubstitutionsPerSet), ("maxTimeoutsPerSet", request.MaxTimeoutsPerSet) })
            if (value is < 1 or > 99) errors.Add(new("invalid_match_rule_limit", path, "Match rule limits must be between 1 and 99."));
        return Task.FromResult(result with { IsValid = errors.Count == 0, Errors = errors });
    }
    public async Task<CompetitionFormatDto> CreateAsync(CreateCompetitionFormatRequest request,CancellationToken ct)
    {await Unique(request.Code,null,ct);EnsureValid(CompetitionFormatDefinitionFactory.Validate(request.MinTeams,request.MaxTeams,request.Definition,request.Code,request.Name));var format=CompetitionFormatDefinitionFactory.Build(request.Code,request.Name,request.Description,request.MinTeams,request.MaxTeams,request.Definition);format.ConfigureMatchRules(request.MaxSubstitutionsPerSet,request.MaxTimeoutsPerSet);format.SetActive(false);repository.Add(format);await unitOfWork.SaveChangesAsync(ct);return ToDto(format,new(0,0));}
    public async Task<CompetitionFormatDto> UpdateAsync(int id,UpdateCompetitionFormatRequest request,CancellationToken ct)
    {
        var format=await Required(id,true,ct);var usage=await repository.GetUsageAsync(id,ct);var structuralChange=format.MaxSubstitutionsPerSet!=request.MaxSubstitutionsPerSet||format.MaxTimeoutsPerSet!=request.MaxTimeoutsPerSet||!string.Equals(format.Code,request.Code?.Trim(),StringComparison.Ordinal)||format.MinTeams!=request.MinTeams||format.MaxTeams!=request.MaxTeams||!DefinitionEquals(ToDefinition(format),request.Definition);
    ...
    private static CompetitionFormatSummaryDto ToSummary(CompetitionFormat x,CompetitionFormatUsage u)=>new(x.CompetitionFormatId,x.Code,x.Name,x.MinTeams,x.MaxTeams,x.Active,u.Used,u.IsStructurallyLocked,u.DraftCompetitionCount,u.OperationalCompetitionCount,x.MaxSubstitutionsPerSet,x.MaxTimeoutsPerSet);
    private static CompetitionFormatDto ToDto(CompetitionFormat x,CompetitionFormatUsage u)=>new(x.CompetitionFormatId,x.Code,x.Name,x.Description,x.MinTeams,x.MaxTeams,x.Active,ToDefinition(x),u.Used,u.IsStructurallyLocked,u.DraftCompetitionCount,u.OperationalCompetitionCount,x.MaxSubstitutionsPerSet,x.MaxTimeoutsPerSet);
