...
public sealed record OpenMatchPlayerRequest(int CompetitionRosterPlayerId,short? JerseyNumber,bool IsMatchCaptain);
public sealed record OpenMatchTeamRequest(IReadOnlyList<OpenMatchPlayerRequest> Players,IReadOnlyList<int> LiberoCompetitionRosterPlayerIds,IReadOnlyList<int> CompetitionRosterStaffIds);
...
public sealed record MatchSheetCurrentStateDto(byte? CurrentSetNumber,byte HomeSets,byte AwaySets,short HomePoints,short AwayPoints,MatchSide? ServingSide,int? ServerMatchPlayerId,short HomeRotationOffset,short AwayRotationOffset,int HomeTimeouts,int AwayTimeouts);
...
public sealed record ScorerSetStateDto(byte SetNumber,string Status,short HomePoints,short AwayPoints,string? InitialServingSide,string? ServingSide,byte HomeRotationOffset,byte AwayRotationOffset,int HomeTimeouts,int AwayTimeouts,string? WinnerSide,IReadOnlyDictionary<string,int[]> Lineups,IReadOnlyList<ScorerSubstitutionStateDto> Substitutions,IReadOnlyList<ScorerLiberoStateDto> LiberoReplacements,IReadOnlyList<string> Points,string? LastSportingEvent,IReadOnlyDictionary<string,ScorerLiberoPlanDto>? LiberoPlans=null,IReadOnlyDictionary<string,int?>? LastLiberoRally=null,IReadOnlyDictionary<string,int?>? LastLiberoRegular=null);
public sealed record ScorerLiberoPlanDto(bool Enabled,int? LiberoMatchPlayerId,IReadOnlyList<int> LogicalPositions);
...
public sealed record ScorerLiberoStateDto(string Side,int Position,int LiberoMatchPlayerId,int ReplacedMatchPlayerId,bool Active,bool Automatic=true);
