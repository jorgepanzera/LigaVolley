...
public sealed partial class MatchEngineService(IMatchSheetRepository sheets, IUnitOfWork unit, PlayoffProgressionService playoffs)
{
    ...
    public Task<MatchEngineCommandResult> RecordSanctionAsync(int matchId, byte setNumber, RecordSanctionRequest request, CancellationToken ct) => Mutate(matchId, async sheet =>
    {
        ...
        var eventType = request.Type switch { SanctionType.MisconductWarning=>MatchEventType.MisconductWarning,SanctionType.MisconductPenalty=>MatchEventType.MisconductPenalty,SanctionType.Expulsion=>MatchEventType.Expulsion,SanctionType.Disqualification=>MatchEventType.Disqualification,SanctionType.ImproperRequest=>MatchEventType.ImproperRequest,SanctionType.DelayWarning=>MatchEventType.DelayWarning,SanctionType.DelayPenalty=>MatchEventType.DelayPenalty,_=>throw Invalid("sanction_type_invalid","Invalid sanction type.")};
        var existing=sheet.Events.SingleOrDefault(x=>x.EventUuid==request.EventUuid); if(existing is not null){if(existing.EventType!=eventType)throw Conflict("sanction_duplicate","EventUuid is already used.");return Result(true,sheet,set);}
        ...
        var now=DateTimeOffset.UtcNow; if(request.Type is SanctionType.MisconductPenalty or SanctionType.DelayPenalty) set.ApplyPoint(request.Side==MatchSide.Home?MatchSide.Away:MatchSide.Home,now);
        sheet.AddEvent(request.EventUuid,eventType,set,request.Side,request.MatchPlayerId,now,staffId:request.MatchTeamStaffId).RecordCommand(System.Text.Json.JsonSerializer.Serialize(request,CommandJson)); sheet.RecalculateSets(); await unit.SaveChangesAsync(ct); return Result(false,sheet,set);
    },ct);
