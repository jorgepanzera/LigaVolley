...
public sealed partial class MatchEngineService(IMatchSheetRepository sheets, IUnitOfWork unit, PlayoffProgressionService playoffs)
{
    ...
    public Task<MatchEngineCommandResult> RecordSanctionAsync(int matchId, byte setNumber, RecordSanctionRequest request, CancellationToken ct) => Mutate(matchId, async sheet =>
    {
        RequiredUuid(request.EventUuid, "sanction_duplicate"); var set = Set(sheet, setNumber); Mutable(sheet); if (set.Status != MatchSetStatus.InProgress) throw Conflict("match_set_invalid_state", "Sanctions require an in-progress set.");
        var eventType = SanctionEventTypes.From(request.Type);
        var existing=sheet.Events.SingleOrDefault(x=>x.EventUuid==request.EventUuid); if(existing is not null){if(existing.EventType!=eventType || existing.CommandPayload is not null && existing.CommandPayload!=System.Text.Json.JsonSerializer.Serialize(request,CommandJson))throw Conflict("sync_event_uuid_conflict","EventUuid was already used with different content.");return Result(true,sheet,set);}
        var team=Team(sheet,request.Side); if(request.SubjectType==SanctionSubjectType.Player && (request.MatchPlayerId is null || !team.Players.Any(x=>x.MatchPlayerId==request.MatchPlayerId))) throw Invalid("sanction_subject_invalid","Player must belong to the selected MatchSheet side.");
        if(request.SubjectType==SanctionSubjectType.Staff && (request.MatchTeamStaffId is null || !team.Staff.Any(x=>x.MatchTeamStaffId==request.MatchTeamStaffId))) throw Invalid("sanction_subject_invalid","Staff must belong to the selected MatchSheet side.");
        if(request.SubjectType==SanctionSubjectType.Team && (request.MatchPlayerId is not null || request.MatchTeamStaffId is not null)) throw Invalid("sanction_subject_invalid","Team sanctions cannot reference a participant.");
        var now=DateTimeOffset.UtcNow; if(SanctionEventTypes.IsPenalty(request.Type)) set.ApplyPoint(request.Side==MatchSide.Home?MatchSide.Away:MatchSide.Home,now);
        sheet.AddEvent(request.EventUuid,eventType,set,request.Side,request.MatchPlayerId,now,staffId:request.MatchTeamStaffId).RecordCommand(System.Text.Json.JsonSerializer.Serialize(request,CommandJson)); sheet.RecalculateSets(); sheet.TouchOperationalState(now); await unit.SaveChangesAsync(ct); return Result(false,sheet,set);
    },ct);
    ...
    private async Task<T> Mutate<T>(int matchId, Func<MatchSheet, Task<T>> action, CancellationToken ct) { await using var tx = await unit.BeginSerializableTransactionAsync(ct); await sheets.AcquireMatchLockAsync(matchId, ct); var sheet = await Required(matchId, ct); var result = await action(sheet); await tx.CommitAsync(ct); return result; }
    ...
    private static bool IsIneligible(MatchSheet sheet, MatchSet set, int matchPlayerId) => sheet.Events.Any(x => x.Status == MatchEventStatus.Active && x.MatchPlayerId == matchPlayerId && (x.EventType == MatchEventType.Disqualification || x.EventType == MatchEventType.Expulsion && x.MatchSetId == set.MatchSetId));
    ...
    private static void ReconcileAutomaticLiberos(MatchSheet sheet, MatchSet set, DateTimeOffset now)
    {
        ...
        {
            var plan = set.LiberoPlans.SingleOrDefault(x => x.MatchTeamId == team.MatchTeamId); if (plan is null) continue;
            if (set.LiberoReplacements.Any(x => x.MatchTeamId == team.MatchTeamId && !x.ExitedAt.HasValue && sheet.Events.Any(e => e.EventUuid == x.ReplacementUuid && e.EventType == MatchEventType.LiberoEnter))) continue;
            var teamLineup = set.Lineups.Single(x => x.MatchTeamId == team.MatchTeamId);
            if (MatchCourtStateCalculator.Calculate(teamLineup, 0, set.Substitutions.Where(x => x.MatchTeamId == team.MatchTeamId), []).Any(x => x.EffectiveMatchPlayerId == plan.LiberoMatchPlayerId)) continue; var offset = team.Side == MatchSide.Home ? set.HomeRotationOffset : set.AwayRotationOffset;
