...
public sealed class MatchEngineService(IMatchSheetRepository sheets,IUnitOfWork unit,PlayoffProgressionService playoffs)
{
    ...
    public Task<MatchEngineCommandResult> SaveLineupAsync(int matchId,byte setNumber,MatchSide side,SetLineupRequest request,CancellationToken ct)=>Mutate(matchId,async sheet=>
    {
        ...
        var lineup=set.Lineups.SingleOrDefault(x=>x.MatchTeamId==team.MatchTeamId);if(lineup is null){lineup=new MatchLineup(set,team);set.Lineups.Add(lineup);}lineup.Replace(players);await unit.SaveChangesAsync(ct);return Result(false,sheet,set);
    },ct);
    ...
    public Task<MatchEngineCommandResult> StartSetAsync(int matchId,byte setNumber,StartSetRequest request,CancellationToken ct)=>Mutate(matchId,async sheet=>
    {
        ...
        var now=DateTimeOffset.UtcNow;set.Start(request.InitialServingSide,now);
    ...
    public Task<MatchEngineCommandResult> AddPointAsync(int matchId,byte setNumber,AddPointRequest request,CancellationToken ct)=>Mutate(matchId,async sheet=>
    {
        ...
        if(sheet.TrackLiberoReplacements&&set.CurrentServingSide!=request.WinningSide){var team=Team(sheet,request.WinningSide);var next=(byte)(((team.Side==MatchSide.Home?set.HomeRotationOffset:set.AwayRotationOffset)+1)%6);var active=set.LiberoReplacements.Where(x=>x.MatchTeamId==team.MatchTeamId&&!x.ExitedAt.HasValue);if(active.Any(x=>MatchCourtStateCalculator.ToPhysical(x.LineupPosition,next) is LineupPosition.P2 or LineupPosition.P3 or LineupPosition.P4))throw Conflict("libero_not_back_row","An active libero must exit before its logical position rotates to the front row.");}
        set.ApplyPoint(request.WinningSide,DateTimeOffset.UtcNow);sheet.AddEvent(request.PointUuid,MatchEventType.Point,set,request.WinningSide,null,DateTimeOffset.UtcNow);sheet.RecalculateSets();await unit.SaveChangesAsync(ct);return Result(false,sheet,set);
    },ct);
    ...
    public Task<MatchEngineCommandResult> CorrectLastPointAsync(int matchId,byte setNumber,CorrectLastPointRequest request,CancellationToken ct)=>Mutate(matchId,async sheet=>
    {
        ...
        sports.Cancel();sheet.AddEvent(request.CorrectionUuid,MatchEventType.PointCorrection,set,null,null,DateTimeOffset.UtcNow,sports);var rebuilt=MatchSetRebuilder.Rebuild(set.InitialServingSide!.Value,sheet.Events.Where(x=>x.MatchSetId==set.MatchSetId));set.Rebuild(rebuilt.Home,rebuilt.Away,rebuilt.Serving,rebuilt.HomeOffset,rebuilt.AwayOffset,DateTimeOffset.UtcNow);sheet.RecalculateSets();await unit.SaveChangesAsync(ct);return Result(false,sheet,set);
    },ct);
    ...
    public Task<MatchEngineCommandResult> SubstituteAsync(int matchId,byte setNumber,AddSubstitutionRequest request,CancellationToken ct)=>Mutate(matchId,async sheet=>
    {
        ...
        var lineup=set.Lineups.Single(x=>x.MatchTeamId==team.MatchTeamId);var offset=team.Side==MatchSide.Home?set.HomeRotationOffset:set.AwayRotationOffset;var court=MatchCourtStateCalculator.Calculate(lineup,offset,set.Substitutions.Where(x=>x.MatchTeamId==team.MatchTeamId),set.LiberoReplacements.Where(x=>x.MatchTeamId==team.MatchTeamId));var slot=court.SingleOrDefault(x=>x.EffectiveMatchPlayerId==outPlayer.MatchPlayerId);if(slot is null)throw Conflict("substitution_player_not_on_court","PlayerOut is not effectively on court.");if(court.Any(x=>x.EffectiveMatchPlayerId==inPlayer.MatchPlayerId))throw Conflict("substitution_player_already_on_court","PlayerIn is already on court.");
        ...
        var substitution=new MatchSubstitution(request.SubstitutionUuid,set,team,outPlayer,inPlayer,slot.LogicalLineupPosition,DateTimeOffset.UtcNow);set.Substitutions.Add(substitution);sheet.AddEvent(request.SubstitutionUuid,MatchEventType.Substitution,set,team.Side,inPlayer.MatchPlayerId,DateTimeOffset.UtcNow);await unit.SaveChangesAsync(ct);return Result(false,sheet,set);
    },ct);
