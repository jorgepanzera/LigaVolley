...
public sealed class OfflineSyncService(IMatchSheetRepository sheets, IUnitOfWork unit, MatchEngineService engine, MatchSheetService snapshots)
{
    ...
    async Task ApplyCore(int id, ScorerSyncEvent e, CancellationToken ct)
    {
        switch (e.Type)
        {
            ...
            case ScorerSyncEventType.Point: { var p = Read<SyncPointPayload>(e); await engine.AddPointCore(id, p.SetNumber, new(e.EventUuid, p.WinningSide, p.ObservedServerMatchPlayerId, p.ConfirmedRuleWarnings), SportingDecisionOrigin.PersistedLocalEvent, ct); break; }
