...
public sealed record UpdateTeamRequest(string Name, Gender Gender);
public sealed record TeamDto(int TeamId, string Name, Gender Gender, ClubSummaryDto? Club, bool Active);
public sealed record TeamSummaryDto(int TeamId, string Name, Gender Gender, int? ClubId, string? ClubName, bool Active, string? ClubLogoUrl);
