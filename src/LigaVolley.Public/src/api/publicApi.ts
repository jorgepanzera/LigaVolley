import type{Competition,CompetitionSummary,Fixture,Live,MatchDetail,Season,SeasonHome,Standings}from'./types';
...
export const publicApi={seasons:()=>get<Season[]>('/seasons'),seasonHome:(id:number)=>get<SeasonHome>(`/seasons/${id}/home`),competitions:(seasonId?:number)=>get<CompetitionSummary[]>(`/competitions${seasonId?`?seasonId=${seasonId}`:''}`),competition:(id:number)=>get<Competition>(`/competitions/${id}`),fixture:(id:number)=>get<Fixture>(`/competitions/${id}/fixture`),standings:(id:number)=>get<Standings>(`/competitions/${id}/standings`),match:(id:number)=>get<MatchDetail>(`/matches/${id}`),live:(id:number,signal?:AbortSignal)=>get<Live>(`/matches/${id}/live`,signal)};
