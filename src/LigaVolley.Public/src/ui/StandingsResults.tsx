...
export function StandingsResults({ standings }: { standings: Standings }) {
  ...
  const groupTables = tablesForPhase.filter(table => table.phaseGroupId !== undefined && table.phaseGroupName);
  ...
  return <><div className="standings-filters" aria-label="Filtros de posiciones">{phases.length > 1 && <label>Fase<select aria-label="Fase" value={selectedPhaseId} onChange={event => update({ phase: event.target.value, group: undefined })}>{phases.map(table => <option key={table.phaseId} value={table.phaseId}>{table.phaseName}</option>)}</select></label>}{hasGroups && <label>Grupo<select aria-label="Grupo" value={selectedTable?.phaseGroupId ?? ''} disabled={!groupTables.length} onChange={event => update({ group: event.target.value || undefined })}>{groupTables.length ? groupTables.map(table => <option key={table.phaseGroupId} value={table.phaseGroupId}>{table.phaseGroupName}</option>) : <option value="">Grupos aún no disponibles</option>}</select></label>}</div>{selectedTable ? selectedTable.rows.length ? <StandingsTableView table={selectedTable} /> : <p className="standings-empty">Todavía no hay posiciones disponibles para esta fase.</p> : <p className="standings-empty">Todavía no hay posiciones disponibles para esta fase.</p>}</>;
}
