...
internal sealed class CompetitionConfiguration : IEntityTypeConfiguration<Competition>
{
    public void Configure(EntityTypeBuilder<Competition> b)
    {
        ...
        b.Property(x => x.PeriodType).AsSql("period_type", 20); b.Property(x => x.StartDate).HasColumnName("start_date").HasColumnType("date"); b.Property(x => x.EndDate).HasColumnName("end_date").HasColumnType("date"); b.Property(x => x.Status).AsSql("status", 20); b.Property(x=>x.ScheduledAt).HasColumnName("scheduled_at").HasColumnType("datetimeoffset"); b.Property(x=>x.CompletedAt).HasColumnName("completed_at").HasColumnType("datetimeoffset");
...
internal sealed class CompetitionPlayoffSeriesConfiguration : IEntityTypeConfiguration<CompetitionPlayoffSeries>
{
    ...
    {
        b.ToTable("PLAYOFF_SERIES", "dbo", t => t.HasCheckConstraint("CK_PLAYOFF_SERIES_winner", "[winner_team_entry_id] IS NULL OR [winner_team_entry_id] IN ([team1_entry_id], [team2_entry_id])"));
        b.ToTable("PLAYOFF_SERIES", "dbo", t => t.HasCheckConstraint("CK_PLAYOFF_SERIES_different_teams", "[team1_entry_id] IS NULL OR [team2_entry_id] IS NULL OR [team1_entry_id] <> [team2_entry_id]")); b.HasKey(x => x.PlayoffSeriesId); b.Property(x => x.PlayoffSeriesId).HasColumnName("playoff_series_id").UseIdentityColumn(); b.Property(x => x.CompetitionId).HasColumnName("competition_id"); b.Property(x => x.CompetitionPhaseId).HasColumnName("competition_phase_id"); b.Property(x => x.FormatSeriesId).HasColumnName("format_series_id"); b.Property(x => x.Code).HasColumnName("code").HasColumnType("varchar(30)"); b.Property(x => x.Name).HasColumnName("name").HasMaxLength(100); b.Property(x => x.Sequence).HasColumnName("sequence"); b.Property(x => x.WinsRequired).HasColumnName("wins_required"); b.Property(x => x.Team1EntryId).HasColumnName("team1_entry_id"); b.Property(x => x.Team2EntryId).HasColumnName("team2_entry_id"); b.Property(x=>x.WinnerTeamEntryId).HasColumnName("winner_team_entry_id"); b.Property(x => x.Team1InitialWins).HasColumnName("team1_initial_wins"); b.Property(x => x.Team2InitialWins).HasColumnName("team2_initial_wins"); b.Property(x => x.Status).AsSql("status",20); b.HasAlternateKey(x => new { x.PlayoffSeriesId, x.CompetitionPhaseId }).HasName("UQ_PLAYOFF_SERIES_id_phase"); b.HasOne(x => x.FormatSeries).WithMany().HasForeignKey(x => x.FormatSeriesId).OnDelete(DeleteBehavior.SetNull); b.HasOne(x=>x.Team1Entry).WithMany().HasForeignKey(x=>new{x.Team1EntryId,x.CompetitionId}).HasPrincipalKey(x=>new{x.TeamEntryId,x.CompetitionId}).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.Team2Entry).WithMany().HasForeignKey(x=>new{x.Team2EntryId,x.CompetitionId}).HasPrincipalKey(x=>new{x.TeamEntryId,x.CompetitionId}).OnDelete(DeleteBehavior.Restrict); b.HasOne(x=>x.WinnerTeamEntry).WithMany().HasForeignKey(x=>new{x.WinnerTeamEntryId,x.CompetitionId}).HasPrincipalKey(x=>new{x.TeamEntryId,x.CompetitionId}).OnDelete(DeleteBehavior.Restrict); b.HasMany(x => x.ParticipantSources).WithOne().HasForeignKey(x => x.TargetPlayoffSeriesId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => new { x.CompetitionPhaseId, x.Code }).IsUnique();
    }
}
