...
{
    public async Task<IReadOnlyList<TeamEntry>> ListAsync(int competitionId, bool tracking, CancellationToken ct)
    {
        IQueryable<TeamEntry> query = db.TeamEntries.Include(x => x.Team);
        if (!tracking) query = query.AsNoTracking();
        return await query.Where(x => x.CompetitionId == competitionId).OrderBy(x => x.Seed == null).ThenBy(x => x.Seed).ThenBy(x => x.Team.Name).ToListAsync(ct);
    }
