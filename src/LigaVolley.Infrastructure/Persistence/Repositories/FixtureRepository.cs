...
internal sealed class FixtureRepository(LigaVolleyDbContext db) : IFixtureRepository
{
    ...
    {
        var query=db.Matches.Include(x=>x.Competition).ThenInclude(x=>x.CompetitionFormat).Include(x=>x.Competition).ThenInclude(x=>x.Season).Include(x=>x.Competition).ThenInclude(x=>x.Division).Include(x=>x.Phase).Include(x=>x.PhaseGroup).Include(x=>x.Series)
            .Include(x=>x.HomeTeamEntry).ThenInclude(x=>x!.Team).ThenInclude(x=>x.Club)
            .Include(x=>x.AwayTeamEntry).ThenInclude(x=>x!.Team).ThenInclude(x=>x.Club)
