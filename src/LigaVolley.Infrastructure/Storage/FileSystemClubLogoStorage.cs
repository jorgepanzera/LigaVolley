...
internal sealed class FileSystemClubLogoStorage : IClubLogoStorage
{
    ...
    public async Task<StoredClubLogo> SaveAsync(int clubId,Stream content,string contentType,CancellationToken ct)
    {
        ...
        using(image){if(image.Width>2048||image.Height>2048)throw new RequestValidationException("club_logo_dimensions_too_large","Logo dimensions cannot exceed 2048x2048.");var expected=image.Metadata.DecodedImageFormat?.DefaultMimeType;if(!string.Equals(expected,contentType,StringComparison.OrdinalIgnoreCase))throw new RequestValidationException("club_logo_invalid_type","Declared content type does not match the image.");image.Mutate(x=>x.Resize(new ResizeOptions{Mode=ResizeMode.Max,Size=new Size(512,512)}));var extension=contentType switch{"image/png"=>"png","image/jpeg"=>"jpg",_=>"webp"};var key=$"clubs/{clubId}/{Guid.NewGuid():N}.{extension}";var path=Resolve(key);Directory.CreateDirectory(Path.GetDirectoryName(path)!);try{await using var output=new FileStream(path,FileMode.CreateNew,FileAccess.Write,FileShare.None,81920,true);if(contentType=="image/png")await image.SaveAsync(output,new PngEncoder(),ct);else if(contentType=="image/jpeg")await image.SaveAsync(output,new JpegEncoder{Quality=85},ct);else await image.SaveAsync(output,new WebpEncoder{Quality=85},ct);return new(key,contentType);}catch(Exception ex)when(ex is IOException or UnauthorizedAccessException){throw new ResourceConflictException("club_logo_storage_error","The logo could not be stored.");}}
    }
