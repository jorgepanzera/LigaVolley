...
export function CompetitionHeader({competition:c}:{competition:Competition}){const tabs=[['Resumen','overview'],['Participantes','participants'],['Fixture','fixture'],['Planteles','rosters'],['Progresión','progression'],...(c.status==='Finished'?[['Movimientos','movements']]:[])];return <><div className="competition-header"><div><h2>{c.name}</h2><p>{c.season.name} · {c.division.name} · {c.format.name}</p></div><StatusBadge status={c.status}/></div><nav className="tabs">{tabs.map(([name,path])=><NavLink key={path} to={`/admin/competitions/${c.competitionId}/${path}`}>{name}</NavLink>)}</nav></>}
